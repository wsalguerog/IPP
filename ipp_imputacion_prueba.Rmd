...
#se tiene una boleta con los precios que no son cero en el mes actual
bol <- precios %>% 
...
```{r}
#se toman los precios que son cero en el mes actual y se revisan si pueden o no imputarse
#estos serian los que si se pueden imputar
sipuede_imp <- precios %>% 
  select(PRODUCTO, VARIEDAD, all_of(mes), all_of(mesant)) %>% 
  filter(!!sym(mes) == 0) %>% 
...
```{r}
#estos serian los que no se pueden imputar
nopuede_imp <- precios %>% 
  select(PRODUCTO, VARIEDAD, all_of(mes), all_of(mesant)) %>% 
  filter(!!sym(mes) == 0) %>% 
  filter(!(PRODUCTO %in% can_imput))
...
```{r}
#estos serian los que no son cero sobre los que se va a calcular el relativo y todo eso para el proceos de imputacion, y si estamos usando los precios del mes actual funciona hacerlo asi como esta aca.
precios_imp <- precios %>% 
  select(PRODUCTO, VARIEDAD, all_of(mes), all_of(mesant)) %>% 
...
precios_imp01 <- precios %>% 
  ...
  group_by(PRODUCTO) %>% 
  summarize(rel_prod = geometric.mean(rel_var),
            mediana = median(!!sym(mes))) %>% 
  mutate(p_imp = rel_prod*mediana)
...
imputados <- sipuede_imp %>% left_join(precios_imp, by = 'PRODUCTO') %>% 
...
```{r}
imputados01 <- sipuede_imp %>% left_join(precios_imp01, by = 'PRODUCTO') %>% 
  mutate(!!sym(mes) := p_imp) %>% 
  select(PRODUCTO, VARIEDAD, !!sym(mes), !!sym(mesant))
imputados01
```
...
boleta <- bol %>% rbind(imputados) %>% 
  rbind(nopuede_imp)
```

```{r}
boleta01 <- bol %>% rbind(imputados01) %>% 
  rbind(nopuede_imp) #esta es usando la mediana de los precios
```

```{r}
#test <- impute_prices(mes, mesant)
test <- imputacion()
```

```{r}
#vamos a hacer una prueba con el indice
relativo_var <- test %>% 
  mutate(rel_var = !!sym(mes) / !!sym(mesant)) %>% 
...
```{r}
#relativo_var
#relativo_var %>% filter(is.infinite(rel_var))
#relativo_var %>% filter(is.na(rel_var))
```

```{r}
#bol %>% filter(!!sym(mes) == 0)
```

```{r}
#bol %>% filter(!!sym(mesant) == 0)
```

```{r}
#bol %>% filter(is.nan(ENERO))
```
...
Esto es usando los precios anteriores
...
```{r}
#aqui se obtendrian los precios anteriores
